...
- [x] 完善卖家登录、商品管理、策略配置、买家访客身份及会话隔离。
- [x] 实现审批表、状态机及同会话单一 PENDING 审批约束；当前查询集中在 Approval Service，尚无重复查询需要抽取 `repositories/`。
- [x] 将审批申请接入 Seller Agent；只有本轮当前正式报价通过后端复核后才能进入 `PENDING`，等待期间的新正式报价会显式撤销旧审批。
- [x] 实现卖家审批页面、权限校验、过期和规则版本检查；审批结果与待处理后续任务标记在同一事务内保存。
- [x] 审批结果和后续回复待处理标记在同一事务内保存；独立 `workers/approval_processor.py` 使用行锁领取待处理或失败任务，重新加载真实业务状态，触发新的 Agent 调用，失败后可重试。
- [x] 实现买家按有效 `offer_id` 明确确认，才允许写入 `AGREED`。
- [x] 用前端增量轮询展示审批后的消息和会话状态；审批与后续回复均使用幂等标识。

#### V2 源码基线与实施原则

V2 不重写 V1，而是在 V1 已验证的价格规则、报价快照、会话隔离、消息幂等和正式回复安全通路上增量扩展。以 V1 完成时的源码为基线，V1 基线边界如下：

- `NegotiationStatus` 已预留 `WAITING_APPROVAL` 和 `AGREED`，`SellerPolicy.version`、`NegotiationSession.version` 也已存在，但还没有审批表、审批 Service、审批 API、审批 Worker 和买家确认接口。
- V1 基线中的 `REQUEST_APPROVAL` 只会重新校验报价并返回“需要卖家确认”的安全提示，不会持久化审批请求，也不会改变会话状态；V2 阶段四已补全创建审批事务。
- 买家身份仍由客户端直接填写 `X-Buyer-ID`，前端固定使用演示买家和会话；它能验证 V1 的会话查询隔离，但不能承担 V2 的真实身份认证与防伪造要求。
- 商品的 `seller_id` 还是演示标识，没有卖家账号、登录态及所有权依赖；商品和策略也只能通过种子数据初始化。
- V1 基线中的 Seller Agent 严格绑定五个工具；V2 阶段四已同步扩展工具清单、上下文绑定、结构化决策测试和正式回复通路。

V2 继续采用**整体规划、分阶段落地、每阶段保持系统可运行**的方式。推荐拆为以下八个阶段。当前阶段一至阶段七已经完成，阶段八尚未开始：

| 阶段 | 主要工作 | 阶段完成标准 |
|---|---|---|
| 1. 身份认证与权限基础（已完成） | 增加最小卖家账号和登录能力，密码只保存安全哈希；由服务端签发并校验卖家登录凭证。买家继续采用轻量访客模式，但访客标识改为服务端签名的令牌或 Cookie，并复用现有 `buyer_id` 保存会话归属。将卖家、买家身份解析集中到 FastAPI 依赖中，业务 Service 只接收已经验证的身份，不信任请求体、模型参数或任意请求头中的身份 ID。迁移现有演示卖家和买家数据时保留可重复执行的种子流程。 | 卖家可以登录和退出；两个浏览器配置文件获得不同的买家访客身份；伪造买家 ID、跨买家读取会话、未登录访问卖家接口、卖家访问他人商品均被拒绝。V1 的咨询和议价链路在新身份方式下仍可运行。 |
| 2. 商品、策略与会话入口（已完成） | 实现商品公开查询、卖家商品新增/编辑/上下架、私有策略读取与更新，以及创建协商会话的 API；补齐对应 Service、Schema 和最小卖家管理页面。所有商品写操作校验卖家所有权；更新价格策略时在同一事务中递增 `policy.version`。买家公开接口不得返回底价、自动接受阈值等私有字段。前端从接口取得商品和会话，不再硬编码 `1001`，但保留种子数据用于演示。 | 卖家可以管理自己的商品与策略，不能操作其他卖家的数据；买家只能看到公开信息并能创建属于自己的会话；规则版本会随修改递增，修改后原有 V1 报价判断仍使用数据库最新规则。 |
| 3. 审批数据模型与事务状态机（已完成） | 增加 `approval_requests` ORM、枚举、Schema 和 Alembic 迁移，保存 `session_id`、不可变 `offer_id`、申请时 `policy_version`、状态、原因、卖家意见、有效期及后续回复处理字段。实现 Approval Service 的创建、取消、过期和状态查询；创建审批时锁定对应会话行，校验当前报价及规则版本，并保证同一会话同一时刻最多一个 `PENDING`。可在迁移验证通过后增加 `pending_session_id` 生成列唯一索引作为数据库兜底。 | 从 V1 数据库升级和从空库全量迁移都成功；两个并发请求不能为同一会话创建两个待审批记录；审批区以外、过期报价、非当前报价和版本不一致的请求均不能进入 `PENDING`；取消或过期后会话状态正确恢复。 |
| 4. Agent 创建审批请求（已完成） | 新增绑定当前会话的 `request_approval` Tool，并将 Seller Agent 从五个 V1 工具扩展为包含审批能力的 V2 工具集。只有本轮买家提交的当前正式报价、交易条件完整且位于审批区时才能创建审批；禁止接受区不能通过 Prompt 或模型决策绕过。成功持久化后才返回“已提交卖家确认”，同时将会话转为 `WAITING_APPROVAL`。处于等待状态时，普通查询可以安全回复；新正式报价必须先显式撤销或失效旧审批，再按新报价重新处理。 | 审批区报价会得到唯一、可查询的 `PENDING` 记录；模型伪造 `offer_id`、虚假审批结果、越权报价或重复申请都不能产生额外审批或正式承诺；模型调用失败时事务回滚，V1 的自动接受、合法还价和禁止区拒绝行为不回归。 |
| 5. 卖家审批 API 与页面（已完成） | 实现待审批列表、详情、同意和拒绝 API，以及卖家审批页面。审批操作重新校验卖家所有权、审批仍为 `PENDING`、未过期、商品仍可协商、报价条件未变化、没有更新的买家条件且策略版本一致。同意或拒绝的结果与 `followup_status`、`followup_request_id` 在同一事务中保存，并对重复请求保持幂等；拒绝或失效后恢复可协商状态。审批通过只代表卖家授权该次特定报价，不直接写入 `AGREED`。 | 正确卖家可查看并处理自己的审批；其他卖家无权访问。过期审批、旧规则审批、已处理审批和条件已变化的审批不能被再次批准；重复点击不会重复变更状态或生成多条后续任务。 |
| 6. 审批后续处理与买家轮询（已完成） | 实现独立的 `workers/approval_processor.py`，领取 `followup_status = PENDING` 或可重试的 `FAILED` 后续任务；领取时使用行锁或条件更新防止多 Worker 重复处理。Worker 重新加载审批、会话、报价和最新规则，触发新的 Seller Agent 调用，通过正式回复安全层写入 Agent 消息，成功后标记 `SENT`，失败后保留可重试状态。消息使用唯一 `followup_request_id` 防重。买家端轮询增量消息和会话状态，不在每个 API Worker 内启动后台轮询，也暂不引入 RabbitMQ、Celery、SSE 或 WebSocket。 | Worker 重复启动、并发领取、模型超时、处理进程中断和服务重启均不会重复发送回复；失败任务可以重试。卖家审批后，买家页面无需刷新整页即可看到审批结果及安全生成的后续消息。 |
| 7. 买家最终确认与会话收口（已完成） | 实现 `/api/negotiations/{id}/confirm` 和关闭会话能力。确认请求必须携带当前 `offer_id` 和幂等标识，并校验买家身份、会话状态、报价归属、有效期、当前性和授权来源：买家可以确认由 Agent 合法生成的当前报价，也可以确认已经被 Agent 自动接受或经卖家审批授权的当前买家报价。校验通过后在事务中持久化确认报价、确认时间和唯一请求标识，写入系统消息并将会话变为 `AGREED`；关闭会话时同步取消待审批。前端仅在存在可确认的有效报价时展示确认入口。 | 审批通过但买家未确认时会话不会进入 `AGREED`；错误买家、旧报价、过期报价、未授权报价和已被新条件替代的报价均不能确认；重复确认只返回既有结果。确认成功后不能继续议价，关闭流程不会遗留有效待审批。 |
| 8. 双端整合与 V2 全量验收 | 整理买家与卖家页面导航，补齐卖家协商列表/详情、审批状态展示、错误提示和轮询生命周期；只在页面复杂度确有需要时再引入 Router、Store 或 Repository，避免预建空抽象。更新种子数据、启动说明、API 文档和 V2 人工验收步骤。执行身份、权限、状态机、金额、并发、幂等、迁移、Worker 恢复和前端构建的完整回归。 | 使用两个独立浏览器窗口可以完成“买家报价 → Agent 申请审批 → 卖家同意/拒绝 → Worker 通知 → 买家确认意向”的可重复演示；拒绝、过期、重复提交、规则更新、新报价、Worker 故障和越权访问均不会误写 `AGREED` 或泄露私有规则。 |
...
阶段 1～3 应优先在不调用真实大模型的情况下验证权限、数据库约束和状态流转；阶段 4～7 先使用可控的模拟决策提供者验证业务闭环，再做一次真实模型冒烟测试。任何审批、确认和状态流转都必须由 Service 根据数据库真实状态决定，Prompt 和模型输出只能提出动作，不能代替授权。

每个 V2 阶段结束前都必须执行自检：复核本阶段的身份、所有权、报价授权和状态机边界；运行受影响的单元测试与 MySQL 集成测试、Ruff、Alembic 从 V1 升级及空库升级检查、前端类型检查和生产构建；检查待提交差异中是否包含真实密钥、登录凭证、临时数据库、模型原始敏感输出或超出阶段范围的文件。涉及事务、幂等或 Worker 的阶段还必须至少执行一次重复请求、并发或故障恢复测试。发现问题后先修复并重新完成自检，再标记该阶段完成。

V2 的终点是记录双方对某个有效报价形成的交易意向，不包含支付、订单履约、库存锁定、跨买家商品独占预订或自动确认实际成交；这些模式 C 能力不能混入 V2。V3 的离线评测与可观测性、V4 的生产部署也继续按后续版本实施。
