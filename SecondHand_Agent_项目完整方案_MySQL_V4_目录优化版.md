...
V2 继续采用**整体规划、分阶段落地、每阶段保持系统可运行**的方式。推荐拆为以下八个阶段。当前阶段一至阶段四已经完成，阶段五尚未开始：
...
| 5. 卖家审批 API 与页面 | 实现待审批列表、详情、同意和拒绝 API，以及卖家审批页面。审批操作重新校验卖家所有权、审批仍为 `PENDING`、未过期、商品仍可协商、报价条件未变化、没有更新的买家条件且策略版本一致。同意或拒绝的结果与 `followup_status`、`followup_request_id` 在同一事务中保存，并对重复请求保持幂等；拒绝或失效后恢复可协商状态。审批通过只代表卖家授权该次特定报价，不直接写入 `AGREED`。 | 正确卖家可查看并处理自己的审批；其他卖家无权访问。过期审批、旧规则审批、已处理审批和条件已变化的审批不能被再次批准；重复点击不会重复变更状态或生成多条后续任务。 |
