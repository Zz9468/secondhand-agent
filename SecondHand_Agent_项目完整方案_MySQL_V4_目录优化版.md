...
- [ ] 审批结果和后续回复待处理标记已在同一事务内保存；阶段六仍需由独立 `workers/approval_processor.py` 领取待处理任务，重新加载真实业务状态，触发新的 Agent 调用，失败后可重试。
...
V2 继续采用**整体规划、分阶段落地、每阶段保持系统可运行**的方式。推荐拆为以下八个阶段。当前阶段一至阶段五已经完成，阶段六尚未开始：
