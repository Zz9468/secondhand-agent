...
V2 继续采用**整体规划、分阶段落地、每阶段保持系统可运行**的方式。推荐拆为以下八个阶段。当前阶段一和阶段二已经完成，阶段三尚未开始：
...
| 3. 审批数据模型与事务状态机 | 增加 `approval_requests` ORM、枚举、Schema 和 Alembic 迁移，保存 `session_id`、不可变 `offer_id`、申请时 `policy_version`、状态、原因、卖家意见、有效期及后续回复处理字段。实现 Approval Service 的创建、取消、过期和状态查询；创建审批时锁定对应会话行，校验当前报价及规则版本，并保证同一会话同一时刻最多一个 `PENDING`。可在迁移验证通过后增加 `pending_session_id` 生成列唯一索引作为数据库兜底。 | 从 V1 数据库升级和从空库全量迁移都成功；两个并发请求不能为同一会话创建两个待审批记录；审批区以外、过期报价、非当前报价和版本不一致的请求均不能进入 `PENDING`；取消或过期后会话状态正确恢复。 |
