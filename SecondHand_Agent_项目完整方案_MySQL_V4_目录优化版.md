...
- `REQUEST_APPROVAL` 目前只会重新校验报价并返回“需要卖家确认”的安全提示，不会持久化审批请求，也不会改变会话状态。
...
- 当前 Seller Agent 严格绑定五个 V1 工具。V2 增加审批工具时，需要同步调整 Agent 工具清单、上下文绑定、结构化决策测试和正式回复通路，不能只新增一个接口或一张表。

V2 继续采用**整体规划、分阶段落地、每阶段保持系统可运行**的方式。推荐拆为以下八个阶段。当前阶段一至阶段三已经完成，阶段四尚未开始：
